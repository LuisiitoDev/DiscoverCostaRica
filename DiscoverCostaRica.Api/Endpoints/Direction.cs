...
public class Direction : IEndpoint
{
	...
	private static async Task<IResult> GetCantons(DirectionService service, CancellationToken cancellationToken)
	{
		var cantons = await service.GetCantons(cancellationToken);
	...
	private static async Task<IResult> GetDistricts(DirectionService service, CancellationToken cancellationToken)
	{
		var districts = await service.GetDistricts(cancellationToken);
